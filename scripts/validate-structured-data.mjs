...
function validateGraph(data, route, errors) {
  if (data?.['@context'] !== 'https://schema.org') {
    errors.push(`${route}: JSON-LD @context must be https://schema.org`);
  }
  if (!Array.isArray(data?.['@graph'])) {
    errors.push(`${route}: JSON-LD must contain an @graph array`);
    return;
  }
  ...
  if (organization) {
    if (organization.legalName !== 'Safety Assurance Global LLC') {
      errors.push(`${route}: Organization legalName mismatch`);
    }
    if (organization.naics !== '541611') {
      errors.push(`${route}: Organization primary NAICS mismatch`);
    }
    if (organization.address?.addressRegion !== 'Oregon' || organization.address?.addressCountry !== 'US') {
      errors.push(`${route}: Organization business-base address mismatch`);
    }
  ...
  if (website?.publisher?.['@id'] !== 'https://safetyassuranceglobal.com/#organization') {
    errors.push(`${route}: WebSite publisher relationship mismatch`);
  }
  ...
  if (breadcrumb) {
    ...
    if (!Array.isArray(items) || items.length < 2) {
      errors.push(`${route}: BreadcrumbList requires at least Home and current page`);
    } else {
      if (items[0]?.position !== 1 || items[0]?.name !== 'Home' || items[0]?.item !== 'https://safetyassuranceglobal.com/') {
        errors.push(`${route}: BreadcrumbList Home item mismatch`);
      }
      items.forEach((item, index) => {
        if (item?.position !== index + 1) errors.push(`${route}: BreadcrumbList positions must be consecutive`);
      });
    }
  }
}
...
try {
  ...
  console.error('Structured-data validation failed: dist/ is missing. Run the Astro build first.');
  process.exitCode = 1;
  throw error;
}
...
for (const file of files) {
  ...
  if (scripts.length === 0) {
    errors.push(`${route}: no application/ld+json script found`);
    continue;
  }
  ...
  for (const raw of scripts) {
    ...
    if (Array.isArray(parsed?.['@graph'])) {
      graphFound = true;
      validateGraph(parsed, route, errors);
    }
  }
